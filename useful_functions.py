...
def remove_hair(im):
    ...
    :return: an image in which we tried to remove hairs
...
def save_network(networks_name, classifier, history, infos_dict):
    ...
    # (futur?) dossier de ce reseau de neurones
    ...
    if not networks_name in subfolders:
        # creer un dossier pour ce reseau de neurones
        os.makedirs(networks_dir)
        print("repertoire [", networks_dir, "] créé")

    # creer le nouveau dossier de test (avec la date et l'heure du test)
    new_test_dir = networks_dir + "/" + date
    if os.path.exists(new_test_dir):
        print("\n\nATTENTION : LE DOSSIER ", new_test_dir, " EXISTE DEJA.\n\n")
...
def display_some_results(model, im_names, threshold=0.3, dim=(320, 320), display=True, save=False):
    ...
    if save:
        ...
        if os.path.exists(saving_path):
            print("\n\nATTENTION : LE DOSSIER ", saving_path, " EXISTE DEJA.\n\n")
...
def draw_contour_on_image(img_name, img_dir, ground_truth, prediction, output_dir):
    ...
    :return: an image of the skin lesion with the two contours
